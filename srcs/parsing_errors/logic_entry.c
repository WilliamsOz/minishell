...
/*   Updated: 2021/12/17 19:36:02 by wiozsert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	heredoc_before_errors(t_minishell *m, t_dlk_list *keep, char **env)
{
	t_dlk_list	*tmp;
...
t_minishell	*is_logic_input(t_minishell *minishell, char **env)
{
	...
	{
		minishell->d_lk = init_heredoc_pipes(minishell, minishell->d_lk, 0);
		heredoc_before_errors(minishell, keep, env);
		close_ununsed_pipes(minishell->d_lk);
		if (minishell->d_lk != NULL)
			minishell->d_lk = double_lk_destroyer(minishell->d_lk);
		if (minishell->line != NULL)
			minishell->line = free_line(minishell->line);
